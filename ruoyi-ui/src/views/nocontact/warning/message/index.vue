  ...
  <div class="app-container warning-message-page">
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="small" :inline="true">
      ...
      <el-form-item label="消息状态">
        <el-select v-model="queryParams.messageStatus" clearable placeholder="全部"><el-option label="已发送" value="sent" /><el-option label="已读" value="read" /><el-option label="已处理" value="handled" /><el-option label="已归档" value="archived" /></el-select>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="messageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" width="190">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-check" @click="changeStatus(scope.row, 'handled')" v-hasPermi="['warning:message:edit']">处理</el-button>
          <el-button size="mini" type="text" icon="el-icon-folder-checked" @click="changeStatus(scope.row, 'archived')" v-hasPermi="['warning:message:edit']">归档</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    statusText(value) {
      const map = { sent: '已发送', read: '已读', handled: '已处理', archived: '已归档' }
    ...
    changeStatus(row, status) {
      updateMessageStatus(row.messageId, status).then(() => {
